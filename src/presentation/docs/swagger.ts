...
import { authSchemas } from "./swagger/auth_schemas";
import {
  ...
  components: {
    ...
    schemas: {
      ...
      RedisModuleHealth: {
        ...
        required: ["active"],
        description:
          "Per-module Redis liveness snapshot used by `GET /health/redis`. `skipped` means the module is opt-in and its URL is empty; `disconnected` means the URL is configured but the connection dropped; `circuit_open` is the short-lived breaker state after repeated command failures.",
        properties: {
          active: {
            type: "boolean",
            description: "True when the module's Redis store is online and ready.",
          },
          reason: {
            type: "string",
            enum: ["skipped", "disconnected", "circuit_open"],
            description: "Set only when active=false. Distinguishes opt-in skip from outage.",
          },
          lastConnectionAtMs: {
            type: "integer",
            minimum: 0,
            description:
              "Timestamp (ms since epoch) of the last successful connect event. 0 = never connected.",
          },
          lastFallbackAtMs: {
            type: "integer",
            minimum: 0,
            description:
              "Timestamp (ms since epoch) of the last fallback event (connect/command failure). 0 = no fallback recorded.",
          },
          circuitOpen: {
            type: "boolean",
            description:
              "Circuit breaker state for modules that have one (socket/REST rate-limit). Omitted for modules without a breaker.",
          },
        },
      },
