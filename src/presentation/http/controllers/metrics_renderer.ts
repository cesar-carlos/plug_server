...
export const buildMetricsLines = (snapshots: MetricsSnapshots): string[] => {
  ...
  const emitRestBridgeAndHttpRateLimit = (): void => {
    ...
    lines.push(
      metricLine("plug_rest_bridge_requests_failed_total", restBridge.requestsFailedTotal),
    );
    ...
    lines.push(
      metricLine(
        "plug_rest_http_rate_limit_client_me_agent_token_put_rejected_total",
        restHttpRl.clientMeAgentTokenPutRejectedTotal,
      ),
    );
    lines.push(
      metricLine(
  ...
  /** Redis-backed module health: REST rate-limit, socket rate-limit, Socket.IO adapter and idempotency store. */
  const emitRedisModuleHealth = (): void => {
    lines.push(
      metricLine(
    ...
    lines.push(
      metricLine(
        ...
        restRateLimitRedis.redisStoreActive,
      ),
    );
    lines.push(
      metricLine(
    ...
    lines.push(
      metricLine(
        ...
        socketRateLimitRedis.redisStoreActive,
      ),
    );
    lines.push(
      metricLine(
  ...
  /** Agent event-stream (Redis Streams), Redis auth-ping probes and Prisma transaction-retry counters. */
  const emitAgentStreamAndInfra = (): void => {
    lines.push(
    ...
    lines.push(
      metricLine("plug_agent_event_stream_batch_size_sum", agentEventStream.batchSize.sum),
    );
    lines.push(
  ...
  /** Registration flow, auth-account guards, agent-profile reliability, data-maintenance and client→agent access. */
  const emitRegistrationAuthAndAgentData = (): void => {
    lines.push(
    ...
    lines.push(
      metricLine(
        "plug_client_me_agents_detail_responses_total",
        clientMeAgents.detailResponsesTotal,
      ),
    );
    lines.push(
      metricLine(
  ...
  /** Socket runtime: namespace connections, consumer/agent auth + runtime counters, payload-frame and hub errors. */
  const emitSocketRuntime = (): void => {
    lines.push(
    ...
    lines.push(
      metricLine(
        "plug_agent_session_rejected_active_total",
        agentRuntime.sessionRejectedActiveTotal,
      ),
    );
    lines.push(
      metricLine(
  ...
  /** Relay engine: request/response counters, stream lifecycle, dispatch + outbound queues and per-agent latency. */
  const emitRelay = (): void => {
    lines.push(
    ...
    lines.push(
      metricLine("plug_socket_relay_chunks_buffered_total", relay.counters.chunksBuffered),
    );
    ...
    lines.push(
      metricLine("plug_socket_relay_frame_decode_sum_ms", relay.counters.frameDecodeSumMs),
    );
    lines.push(
      metricLine(
    ...
    lines.push(
      metricLine(
        ...
        relay.counters.commandValidateCount > 0
          ? Number(
              (relay.counters.commandValidateSumMs / relay.counters.commandValidateCount).toFixed(
                4,
              ),
            )
          : 0,
    ...
    lines.push(
      metricLine("plug_socket_relay_bridge_encode_count", relay.counters.bridgeEncodeCount),
    );
    lines.push(
    ...
    lines.push(
      metricLine(
        ...
        relay.counters.chunkForwardJobCount > 0
          ? Number(
              (relay.counters.chunkForwardJobSumMs / relay.counters.chunkForwardJobCount).toFixed(
                4,
              ),
            )
          : 0,
    ...
    lines.push(
      metricLine("plug_socket_relay_buffer_drain_sum_ms", relay.counters.bufferDrainSumMs),
    );
    lines.push(
      metricLine(
        ...
        relay.counters.bufferDrainRunCount > 0
          ? Number(
              (relay.counters.bufferDrainSumMs / relay.counters.bufferDrainRunCount).toFixed(4),
            )
          : 0,
      ),
    );
  ...
  /** Socket rate limits: relay conversation/request/stream-pull credits, agents-command and client-event publish. */
  const emitRateLimits = (): void => {
    ...
    lines.push(
      metricLine(
        "plug_socket_agents_command_rate_limit_allowed_total",
        agentsCommandRl.allowedTotal,
      ),
    );
    lines.push(
      metricLine(
  ...
  /** Socket audit persistence, bridge-latency trace persistence and HTTP RED (requests/in-flight/duration). */
  const emitAuditBridgeLatencyAndHttpRed = (): void => {
    ...
    lines.push(
      metricLine("plug_bridge_latency_trace_prune_failed_total", bridgeLatency.pruneFailed),
    );
  ...
  emitRestBridgeAndHttpRateLimit();
  emitRedisModuleHealth();
  emitAgentStreamAndInfra();
  emitRegistrationAuthAndAgentData();
  emitSocketRuntime();
  emitRelay();
  emitRateLimits();
  emitAuditBridgeLatencyAndHttpRed();
