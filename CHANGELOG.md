...
- **Inbound PayloadFrame**: `decodePayloadFrameAsync` com `PAYLOAD_FRAME_ASYNC_GUNZIP_MIN_COMPRESSED_BYTES` (defeito **131072**): `gunzip` assincrono para frames `cmp: gzip` grandes em `rpc:response`, acks e relay consumer decode. **`rpc:chunk` / `rpc:complete` mantem `decodePayloadFrame` sync** para preservar ordem por socket. `decodePayloadFrame` sync em `agent:register` / heartbeat em `socket.ts`.
- **Auditoria**: `SOCKET_AUDIT_HIGH_VOLUME_SAMPLE_PERCENT` (0–100, defeito **100**) — amostragem aleatoria de eventos `relay:rpc.chunk` antes de enfileirar/INSERT; metrica `plug_socket_audit_writes_sample_skipped_total`.
...
- **Defaults**: `SOCKET_REST_STREAM_PULL_WINDOW_SIZE` **128**; `SOCKET_AUDIT_BATCH_MAX` **48**; `SOCKET_AUDIT_BATCH_FLUSH_MS` **200**.
...
- **Desempenho Socket.IO**: `serveClient` defeito `false`; `SOCKET_IO_HTTP_COMPRESSION` configuravel (defeito `true`); `SOCKET_IO_PING_INTERVAL_MS` / `SOCKET_IO_PING_TIMEOUT_MS` opcionais; documentado em `performance_hub_agent.md` e `configuration.md`.
...
- **Defaults de throughput**: `SOCKET_REST_STREAM_PULL_WINDOW_SIZE` **128** (antes 96), `SOCKET_REST_AGENT_MAX_INFLIGHT` **24**, `SOCKET_REST_AGENT_MAX_QUEUE` **48**, `SOCKET_RELAY_RATE_LIMIT_MAX_REQUESTS` **64**, buffers relay **128** / **12800** chunks.
