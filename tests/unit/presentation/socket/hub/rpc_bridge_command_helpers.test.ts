...
describe("rpc_bridge_command_helpers", () => {
  ...
  it("resolveOutboundApiVersion trims or defaults to 2.10", () => {
    expect(resolveOutboundApiVersion({ api_version: "  3.0  " })).toBe("3.0");
    expect(resolveOutboundApiVersion({ api_version: "" })).toBe("2.10");
    expect(resolveOutboundApiVersion({})).toBe("2.10");
  });
  ...
  it("withBridgeMeta preserves per-item api_version in batch", () => {
    ...
    expect(out[1].api_version).toBe("2.10");
