...
| `AGENT_HUB_PRESENCE_REDIS_URL`                          | _(vazio → `SOCKET_IO_REDIS_ADAPTER_URL`)_ | Redis para presenca distribuida do agente e forward de `POST /api/v1/agents/commands` entre replicas. Vazio e adapter vazio = desactivado (registry local apenas). |
