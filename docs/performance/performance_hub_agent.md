...
- **Forward sem re-gzip**: quando o body JSON-RPC não é mutado, o hub pode reutilizar `wireBytes` comprimidos do agente (`encodePayloadFrameFromPreencodedWire`) em vez de gunzip→gzip; chunks com `rawForward` bufferizam só bytes + meta (rowCount) sob backpressure.
- **Presence Redis**: `syncAgentHubPresenceOnTouch` renova TTL no Redis no máximo a cada `AGENT_HUB_PRESENCE_TTL_MS / 3` (liveness local no heartbeat continua incondicional).
- **Heartbeat**: decode síncrono (`decodePayloadFrame`) para frames de controlo pequenos; ack continua em `encodePayloadFrameHotPath`.
...
- **`max_rows` efetivo**: o bridge aplica clamp de `options.max_rows` ao limite negociado por agente (mínimo entre contrato do hub e limite anunciado pelo agente, quando presente), reduzindo risco de pedidos acima da capacidade declarada.
...
| `REST_SOCKET_EVENT_IDEMPOTENCY_REDIS_URL` | Redis opcional para replay/conflito e lock da primeira emissao de `client:custom.*` entre replicas. Use junto de `SOCKET_IO_REDIS_ADAPTER_URL` quando publicar com idempotencia em ambiente multi-replica. |
...
| `AGENT_SQL_BULK_INSERT_MAX_ROWS` / `AGENT_SQL_BULK_INSERT_MAX_JSON_BYTES` | Caps antes de montar `PayloadFrame` para `sql.bulkInsert` (defeitos **50000** linhas / **10 MiB** de JSON serializado em `params`); cargas maiores devem ser loteadas pelo cliente. |
...
| `PAYLOAD_FRAME_ASYNC_GUNZIP_MIN_COMPRESSED_BYTES` | Gunzip assíncrono inbound (defeito **1** byte comprimido); `0` = só síncrono. |
